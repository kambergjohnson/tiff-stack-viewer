...
def getImages(fN):
	...
	num_channels = int(settings['channels'] if settings.has_key('channels') else '1')
	num_stacks = int(settings['slices'])
	num_time = int(int(settings['frames'] if settings.has_key('frames') else '1'))
...
def showVolume(inList):
	...
	if viewMode == 'v':
		...
		for zI in range(stretched.shape[0]):
			minI = np.floor(zI/scale)
			maxI = np.ceil(zI/scale)
			weight = zI/scale-minI
